/*
 * Project: Homeyduino
 * Author: Renze Nicolai <renze@rnplus.nl>
 * License: GPLv3
 *
 */

"use strict";

const events = require('events');
const rp = require('request-promise-native');
const express = require('express');
const http = require('http');
const bodyParser = require('body-parser');
const promiseRetry = require('promise-retry');

class ArduinoDevice extends events.EventEmitter {

	constructor( opts ) {
		super();

		this._opts = Object.assign({
			id: 'unknown',
			type: 'unknown',
			address: '0.0.0.0',
			port: 46639,
			api: [],
			localPort: 0,
			localAddress: '0.0.0.0',
			subscribed: false,
			debug: false,
			debugEmit: false,
			lastSeen: null,
			paired: false,
			rc: null
		}, opts);

		this._app = express();
		this._webserver = http.createServer({ requireHostHeader: false }, this._app);

		this._webserver.on('error', (err) => {
			this._debug('Webserver error on device ' + this._opts.id + ':', err.message || err);
		});

		this._app.use(bodyParser.json());

		this._app.use((err, req, res, next) => {
			if (err) {
				this._debug('Express body parse error on device ' + this._opts.id + ':', err.message || err);
				return res.status(400).send('error');
			}
			next();
		});

		this._app.get('/', (req, res) => {
			this._debug('Webserver: index requested');
			res.send('');
		});

		this._app.post('/emit/*', (req, res) => {
			try {
				let urlParts = req.url.split('/');
				let emType = urlParts[2];
				let name = urlParts[3];

				let argument = req.body ? req.body.argument : undefined;
				let type = req.body ? req.body.type : undefined;
				this.emit('emit', {"name":name, "emType":emType, "type":type, "argument":argument});
				res.send('ok');
			} catch(e) {
				this._debug('Emit request error: invalid data', e.message || e);
				res.send('error');
			}
		});

		this._app.get('*', (req, res) => {
			this._debug('Ignored unhandled GET request: '+req.url);
			res.status(404).send('Unknown (GET)');
		});

		this._app.post('*', (req, res) => {
			this._debug('Ignored unhandled POST request: '+req.url);
			res.status(404).send('Unknown (POST)');
		});

		try {
			this._webserver.listen(0, () => {
				try {
					let addr = this._webserver.address();
					if (addr && typeof addr === 'object') {
						this._opts.localPort = addr.port;
					}
				} catch (e) {
					this._debug('Error reading localPort:', e.message || e);
				}
			});
		} catch (e) {
			this._debug('Webserver listen exception:', e.message || e);
		}
	}

	destroy() {
		this.removeAllListeners();
		if (this._webserver) {
			try {
				this._webserver.close();
			} catch (e) {}
		}
	}

	update ( opts ) {
		this._opts.lastSeen = opts.lastSeen; //Update last discovery moment
		if ( opts.master && JSON.stringify(this._opts.master) != JSON.stringify(opts.master) ) {
			this._debug("Device master changed to "+opts.master.host+":"+opts.master.port);
			this._opts.master = opts.master;
			this.emit('master', {"device":this, "master":this._opts.master});
		}
		if ( this._opts.subscribed && opts.master && JSON.stringify({"host":this._opts.localAddress,"port":this._opts.localPort}) != JSON.stringify(opts.master) ) {
			this._debug("Lost subscription, subscribing...");
			this.subscribe().then(() => {
				console.log('Re-registered with device.');
			}).catch((err) => {
				console.log('Lost registration with device:', err.message || err);
			});
		}
		if ( JSON.stringify(this._opts.api) != JSON.stringify(opts.api) ) {
			this._debug("API changed");
			this._opts.api = opts.api;
			this.emit('api', {"device":this, "api":this._opts.api});
		}
		if (( this._opts.address != opts.address ) || ( this._opts.port != opts.port )) {
			this._debug("Network location changed ("+this._opts.address+ ":"+this._opts.port+" to "+opts.address+":"+opts.port+")");
			this._opts.address = opts.address;
			this._opts.port = opts.port;
			this.emit('network', {"device":this, "address":this._opts.address, "port":this._opts.port});
		}

		//Update "static" info (in case someone swaps to a different board with same id and ip address)
		this._opts.type = opts.type;
		this._opts.class = opts.class;
		this._opts.arch = opts.arch;
		this._opts.numDigitalPins = opts.numDigitalPins;
		this._opts.numAnalogInputs = opts.numAnalogInputs;
		this._opts.rc = opts.rc;
	}

	_debug() {
		if( this._opts.debug ) {
			console.log.apply( null, arguments );
		}
		if( this._opts.debugEmit ) {
			this.emit('debug', arguments );
		}
	}
	
	libVersion() {
		if ('version' in this._opts) {
			return this._opts.version;
		} else {
			return "0.0.0";
		}
	}
	
	hasRc() {		
		if ('rc' in this._opts) {
			if (this._opts.rc == "0") {
				this._debug("Warning: device is using an outdated pre-release version of the Arduino library.");
				return false;
			}
			if (this._opts.rc != null) {
				return true;
			}
		}
		return false;
	}

	executeRequest(path, body, get=false){

		var rq = {};
		var rt = 1;

		if (get) {
			rq = {
				method	: 'GET',
				uri		: 'http://' + this._opts.address+ ':' + this._opts.port + '/' + path + '?' + body,
				headers	: {},
				timeout : 2000
			};
			rt = 3; //Try GET requests 3 times
		} else {
			rq = {
				method	: 'POST',
				uri		: 'http://' + this._opts.address+ ':' + this._opts.port + '/' + path,
				body	: body,
				headers	: {},
				timeout : 10000
			};
			rt = 1; //Try POST requests only once
		}

		return promiseRetry( (retry, number) => {
			if (number>1) this._debug('Request failed, try', number);
			return rp(rq).catch( (err) => {
				retry(err);
			});
		}, {retries: rt}).then((result) => {
			try {
				let parsedBody = JSON.parse(result);
				return Promise.resolve(parsedBody);
			} catch(e){
				this._debug("executeRequest: could not decode json response");
				return Promise.reject(result);
			}
		}).catch((err) => {
			try {
				if (err && err.error) {
					let parsedError = JSON.parse(err.error);
					return Promise.reject(parsedError);
				}
			} catch(e){}
			return Promise.reject(err);
		});
	}

	query( name, type, parameter="", get=false ) {
		let endpoint = type+"/"+name;
		return this.executeRequest(endpoint, parameter, get)
			.then( (body) => {
				if ( body && typeof body === 'object' && ('r' in body) ) {
					if ( ('t' in body) && body.t === "err" ) {
						return Promise.reject( body.r );
					} else {
						return Promise.resolve( body.r );
					}
				} else {
					return Promise.reject( 'query: no result in response!!!' );
				}
			})
			.catch( (err) => {
				if ( err && typeof err === 'object' && ('r' in err) ) {
					return Promise.reject( err.r );
				} else {
					return Promise.reject( err );
				}
			});
	}

	setLocalAddress( addr ) {
		this._opts.localAddress = addr;
	}

	subscribe() {
		return this.executeRequest("sys/setmaster", this._opts.localAddress+":"+this._opts.localPort)
			.then( (body) => {
				if ( body && typeof body === 'object' && ('r' in body) ) {
					if ( ('t' in body) && body.t === "err" ) {
						return Promise.reject( body.r );
					} else {
						this._opts.subscribed = true;
						return Promise.resolve( body.r );
					}
				} else {
					return Promise.reject( 'setmaster: no result in response!!!' );
				}
			})
			.catch( (err) => {
				return Promise.reject(err);
			});
	}

	unsubscribe() {
		this._opts.subscribed = false;
	}

	getOpt( key ) {
		return this._opts[ key ];
	}

	setOpt( key, value ) {
		this._opts[ key ] = value;
	}
}

module.exports = ArduinoDevice;

